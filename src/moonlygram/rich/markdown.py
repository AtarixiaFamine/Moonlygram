...
def markdown_to_rich(text: str) -> str:
    ...
    # GitHub-style tables → <table bordered striped>
    ...
    # inline code that may sit inside one of its cells.
...
def _convert_tables(text: str, stash: Callable[[str], str]) -> str:
    ...
    while i < len(lines):
        ...
        if (
            ...
            head_html = "".join(
                f"<th>{html.escape(c, quote=False)}</th>" for c in header
            )
            body_html = ""
            for row in rows:
                ...
                body_html += (
                    "<tr>"
                    + "".join(f"<td>{html.escape(c, quote=False)}</td>" for c in cells)
                    + "</tr>"
                )
            out.append(
                stash(
